...
export class Color extends MenuItem {
                ...
                if (nodes.length > 1) {
                    nodes.forEach((node, index) => {
                        ...
                        if (index === 0) {
                            let html = `${textContent.slice(range.startOffset)}`;
                            if (styles.length) {
                                html = `<span style="color: ${color};${styles.join(';')}">${html}</span>`;
                            }
                            node.insertAdjacentHTML('afterend', html);
                    ...
                    if (!curColor) {
                        ...
                        if (!text) {
                            ...
                            if (textContent.startsWith(text)) {
                                ...
                                node.insertAdjacentHTML('afterend', `<span style="color: ${color};${styles.join(';')}">${text}</span>`);
                                node.innerHTML = node.innerHTML.replace(text, '');
                            } else {
                                const [ textBefore, textAfter ] = textContent.split(text);
                                node.insertAdjacentHTML('beforebegin', `<span style="color: ${curColor};${styles.join(';')}">${textBefore}</span>`);
                                node.insertAdjacentHTML('afterend', `<span style="color: ${curColor};${styles.join(';')}">${textAfter}</span>`);
                                node.innerHTML = `<span style="color: ${color};${styles.join(';')}">${text}</span>`;
                            }
                        }
                    }
                }
